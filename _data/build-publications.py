...
# Newly posted works can take a few days to appear in OpenAlex. Keep verified
# records here so they are not dropped from the site during that indexing lag.
# Once OpenAlex has a matching DOI, its record takes precedence automatically.
CURATED_PUBLICATIONS = [
    {
        "id": "doi:10.64898/2026.07.28.741211",
        "title": (
            "seqproc: An efficient, flexible, and concise tool for sequence geometry "
            "description and transformation"
        ),
        "authors": ["Noah Cape", "Elan Fisher", "Daniel Liu", "Rob Patro"],
        "num_authors": 4,
        ...
        "type": "preprint",
        "is_preprint": True,
        "doi": "https://doi.org/10.64898/2026.07.28.741211",
        "link": "https://doi.org/10.64898/2026.07.28.741211",
        "is_oa": True,
        "oa_url": (
            "https://www.biorxiv.org/content/10.64898/"
            "2026.07.28.741211v1.full.pdf"
        ),
    },
]
...
def normalized_doi(publication):
    return (publication.get("doi") or "").strip().lower().removeprefix("https://doi.org/")
...
def main():
    ...
    # Add curated records only while OpenAlex does not have their DOI.
    seen_dois = {normalized_doi(pub) for pub in pubs if normalized_doi(pub)}
    for publication in CURATED_PUBLICATIONS:
        doi = normalized_doi(publication)
    ...
    header = (
        "# Generated by _data/build-publications.py from OpenAlex and curated additions. "
        "Do not edit by hand.\n"
